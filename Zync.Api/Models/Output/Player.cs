namespace Zync.Api.Models.Output
{

    public class Players
    {
        public List<Player> PlayersList { get; set; } = new List<Player>();
    }

    public class Player
    {
        public string ID { get; set; }
        public string BsPlayerId { get; set; }
        public string BsFrameId { get; set; }
        public double Latitude { get; set; }
        public double Longitude { get; set; }
        public string City { get; set; }
        public string Country { get; set; }
        public string PlayerName { get; set; }
        public string DisplayUnitId { get; set; }
        public string PanelId { get; set; }
        public string FaceId { get; set; }
        public string DisplayUnitName { get; set; }
        public string Status { get; set; }
        public string Frames { get; set; }
        public string FrameName { get; set; }
        public string GeometryType { get; set; }
        public string FrameWidth { get; set; }
        public string FrameHeight { get; set; }
        public string Community { get; set; }
        public string Province { get; set; }
        public string Zone { get; set; }
        public string District { get; set; }
        public string Neightborhood { get; set; }
        public string Address { get; set; }
        public string Street { get; set; }
        public string StreetNumber { get; set; }
        public string PostalCode { get; set; }
        public string Format { get; set; }
        public string Furniture { get; set; }
        public string Connect { get; set; }
        public string BusStation { get; set; }
        public string TrainStation { get; set; }
        public string SubwayStation { get; set; }
        public string PlayerClass { get; set; }
        public string TrafficOrigin { get; set; }
        public string TrafficDestination { get; set; }
        public string TrafficBMFast { get; set; }
        public string TrafficBMNormal { get; set; }
        public string TrafficBMSlow { get; set; }
        public string WifiProbesGroup { get; set; }
        public string LanguageFR { get; set; }
        public string LanguageNL { get; set; }
        public string Geohash { get; set; }
        public string AffluenceSiteId { get; set; }
        public string SonataLocation { get; set; }
        public string AppcelerateLocation { get; set; }
        public string Group { get; set; }
        public string BusStopName { get; set; }
        public string BusStopId { get; set; }
        public string ClientId { get; set; }
        public string UpdatedAt { get; set; }
        public string BusStopPanel { get; set; }
        public string Name { get; set; }
        public string FrameId { get; set; }
    }
}
